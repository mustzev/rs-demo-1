use cornucopia::{CodegenSettings, generate_live};
...
fn main() -> Result<(), cornucopia::Error> {
    dotenvy::from_filename("env/.env.dev").expect("Cannot load env variables");

    let queries_path = "src/cornucopia/queries-inventory";
